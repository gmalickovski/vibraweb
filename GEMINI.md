...
- Faz chamadas ao Neon usando Neon Auth + Data API.
...
**`backend/` (Neon e migrations):**
- Contém migrations e scripts de apoio para Lakebase Postgres no Neon.
...
- Neon Functions usam variáveis de ambiente do servidor; credenciais de banco/storage nunca vão para o browser.
...
| `feature-preview-document.md` | **Estrutura de blocos do documento**, renderização, impressão, textos do Neon |
| `feature-marketing-site.md` | Landing page, seções, CTAs |
| `feature-neon.md` | Backend Neon, schema, RLS, Auth, Storage, Functions e Data API |
...
**Regra de renderização**: blocos condicionais (que dependem de dados calculados) aparecem automaticamente conforme os dados existem. Textos são adicionados no Neon; o sistema cria as páginas automaticamente.
